      ...
      <el-scrollbar :wrapStyle="[{maxHeight:bodyHeight - 123 + 'px'}]">
        <div class="dialog-content" >
          ...
          >
            ...
            <el-form-item label="签发人" prop="signSendUser">
              <el-input v-model="form.signSendUser" placeholder="请输入"></el-input>
            </el-form-item>
            <el-form-item label="发往单位" prop="signReceiveCompany">
            ...
            <el-form-item prop="adjunctAdress" v-if="isAdd===1">
              ...
              v-loading="uploading" accept=".pdf,.doc,.docx" :on-remove="handle_remove">
                <el-button type="text">+添加发文原文附件</el-button>
              </el-upload>
            </el-form-item> 
            <el-form-item label="发文原文附件" prop="" v-if="isAdd===2">
  ...
  methods: {
    ...
    handleSubmit(val) {
      if (val==1) {
        ...
        this.$refs['submitForm'].validate(valid => {
        if (valid) {
          // console.log(this.rowListUpdate)
          this.form.adjunctAdress=this.imgList.join(',');
          this.form.sendType=0;
