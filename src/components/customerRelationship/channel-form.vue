      ...
      <el-scrollbar :wrapStyle="[{maxHeight:bodyHeight - 123 + 'px'}]">
        <div class="dialog-content">
          ...
          >
            ...
            <el-form-item label="渠道来源" prop="parentId">
              <el-select v-model="formData.parentId" filterable placeholder="请选择" @change="channelFromChange">
                <el-option
            ...
            <el-form-item label="附件" prop="attachments" v-if="isAdd==2">
              <el-button type="text" v-for="(item,index) in formData.attachments" :key="index" @click="yuLan(item)" title="点击下载"
...
export default {
  ...
  data() {
    return {
        searchForm:{page: 1, pageSize: 100,type:1},
        businessList:[],
        formRules: {},
        // fileList:[],
        // fiList:[],
  ...
  methods: {
    ...
    //     this.fiList.push(res[0]);
    //     if (this.isAdd==3) {
    //       this.formData.attachments=this.fiList;
    //     }
    //   },
      handleSubmit(val) {
      if (val==1) {
          ...
          if (valid) {
            ...
            businessServer.businessAdd(this.formData,res=>{
              ...
              this.$emit('submit-success',this.formData.parentId);  
              this.$emit('close-dialog'); 
            },()=>{})      
